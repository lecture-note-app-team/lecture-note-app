...
// gpt-4.1-mini は入出力どちらも安価なモデルなので、個人利用ではこのままで十分。
const AI_MODELS = {
...
const handleGenerateQuiz = [
  ...
  wrap(async (req, res) => {
    ...
    if (note.ai_quiz_generated_at) {
      return res.status(409).json({
        message: "このノートのAIクイズ自動生成はすでに実行済みです（1ノートにつき1回のみ）。個別にクイズを追加したい場合は手動作成をご利用ください。",
        code: "AI_QUIZ_ALREADY_GENERATED",
        generatedAt: note.ai_quiz_generated_at,
      });
    }
    ...
    await pool.query("UPDATE notes SET ai_quiz_generated_at = NOW() WHERE id = ?", [noteId]);
...
app.post(
  ...
  wrap(async (req, res) => {
    ...
    let distractors = [];
    try {
      const response = await openai.responses.create({ model: AI_MODELS.quiz, input: prompt });
...
// ---------- Review Notifications (間隔反復の復習通知) ----------
// 各ステージ経過後、次のリマインドまでの日数
const REVIEW_INTERVAL_DAYS = [1, 3, 7, 14, 30];

app.get("/api/reviews/due", requireLogin, wrap(async (req, res) => {
  const userId = req.session.userId;
  const [rows] = await pool.query(
    `SELECT rs.note_id, rs.stage, rs.next_review_at, rs.last_reviewed_at,
            n.title, n.course_name, n.lecture_no
       FROM note_review_schedules rs
       JOIN notes n ON n.id = rs.note_id
      WHERE rs.user_id = ? AND rs.next_review_at <= NOW()
...
app.post("/api/reviews/:noteId/done", requireLogin, wrap(async (req, res) => {
  const userId = req.session.userId;
  const noteId = Number(req.params.noteId);

  const [rows] = await pool.query(
    `SELECT stage FROM note_review_schedules WHERE user_id = ? AND note_id = ? LIMIT 1`,
    [userId, noteId]
  );
  const currentStage = rows.length ? Number(rows[0].stage) : 0;
  const nextStage = Math.min(currentStage + 1, REVIEW_INTERVAL_DAYS.length - 1);
  const intervalDays = REVIEW_INTERVAL_DAYS[nextStage];

  await pool.query(
    `INSERT INTO note_review_schedules (user_id, note_id, stage, next_review_at, last_reviewed_at)
     VALUES (?, ?, ?, DATE_ADD(NOW(), INTERVAL ? DAY), NOW())
     ON DUPLICATE KEY UPDATE
       stage = VALUES(stage),
       next_review_at = VALUES(next_review_at),
       last_reviewed_at = VALUES(last_reviewed_at)`,
    [userId, noteId, nextStage, intervalDays]
  );

  res.json({ ok: true, stage: nextStage, nextReviewInDays: intervalDays });
}));

// ---------- Mind Map (マインドマップ: AI自動生成 + 手動編集) ----------

// ツリーJSON（title/children再帰）をノード＋エッジのレイアウト座標に変換
function layoutMindmapTree(tree) {
  const nodes = [];
  const edges = [];
  let leafCounter = 0;
  const LEVEL_X = 240;
  ...
  function walk(node, depth, parentId) {
    ...
    // 一旦仮のyを入れ、子を積んだ後に中央寄せする
    const startLeafIndex = leafCounter;
    ...
    const placeholder = { id, label: String(node.title || "").slice(0, 60), x: depth * LEVEL_X, y: 0, parentId };
    ...
    const endLeafIndex = children.length ? leafCounter - 1 : startLeafIndex;
    placeholder.y = ((startLeafIndex + endLeafIndex) / 2) * LEAF_Y;
  }
...
async function generateMindmapTreeWithAI(note) {
  const openai = getOpenAIClient();
  const body = String(note?.body_raw || "").slice(0, 8000);

  const prompt = [
    "次の講義ノートの内容から、学習用マインドマップを作成してください。",
    "出力はJSONのみ。余計な文章は禁止。",
    "構造: { \"title\": \"中心テーマ\", \"children\": [ { \"title\": \"トピック\", \"children\": [ { \"title\": \"詳細\" } ] } ] }",
    "ルール:",
    "- 中心(title)は授業・ノートの主題を短く。",
    "- 第1階層は3〜6個の主要トピック。",
    "- 第2階層（必要なら第3階層まで）は各トピックの要点・用語・具体例。",
    "- 各titleは日本語で15文字以内を目安に簡潔に。",
    "- 本文に書かれていない情報を作り話しない。",
    "",
    `【タイトル】${note.title || ""}`,
    `【授業名】${note.course_name || ""}`,
    "【本文】",
    body,
  ].join("\n");

  const resp = await openai.chat.completions.create({
    model: AI_MODELS.mindmap,
    temperature: 0.3,
    response_format: { type: "json_object" },
    messages: [
      { role: "system", content: "あなたは学習教材のマインドマップ作成者です。JSONのみ返答してください。" },
  ...
  try {
    ...
    throw new Error("AIの返答がJSONとして解析できませんでした");
  }
  if (!parsed || typeof parsed.title !== "string") {
    throw new Error("マインドマップの生成結果が不正です");
  }
  return parsed;
}

// 取得（本人 or 閲覧権限があれば可）
app.get("/api/notes/:id/mindmap", wrap(async (req, res) => {
  const noteId = Number(req.params.id);
  const note = await getNoteById(noteId);
  const perm = await canViewNote(req, note);
  if (!perm.ok) return res.status(perm.status).json({ message: perm.message });
  ...
  const [rows] = await pool.query(
    "SELECT note_id, user_id, data, source, updated_at FROM note_mindmaps WHERE note_id = ? LIMIT 1",
    [noteId]
  );
  if (!rows.length) return res.status(404).json({ message: "マインドマップはまだ作成されていません" });
...
// AIで自動生成（作成者のみ／既存があれば上書き）
app.post("/api/notes/:id/mindmap/generate", requireLogin, wrap(async (req, res) => {
  const noteId = Number(req.params.id);
  const note = await getNoteById(noteId);
  const perm = canEditNote(req, note);
  if (!perm.ok) return res.status(perm.status).json({ message: perm.message });
  ...
  const rawText = String(note?.body_raw || "").trim();
  if (rawText.length < 20) {
    return res.status(422).json({ message: "ノート本文が短すぎるためマインドマップを生成できません。" });
  }

  const tree = await generateMindmapTreeWithAI(note);
  const layout = layoutMindmapTree(tree);

  await pool.query(
    `INSERT INTO note_mindmaps (note_id, user_id, data, source)
     VALUES (?, ?, ?, 'ai')
     ON DUPLICATE KEY UPDATE data = VALUES(data), source = 'ai', updated_at = CURRENT_TIMESTAMP`,
    [noteId, req.session.userId, JSON.stringify(layout)]
  );

  res.json({ ok: true, data: layout });
}));

// 手動編集の保存（ノード位置・ラベルの変更、ノード追加削除など全体を上書き保存）
app.put("/api/notes/:id/mindmap", requireLogin, wrap(async (req, res) => {
  const noteId = Number(req.params.id);
  const note = await getNoteById(noteId);
  const perm = canEditNote(req, note);
  if (!perm.ok) return res.status(perm.status).json({ message: perm.message });
  ...
  if (!nodes || !edges) {
    return res.status(400).json({ message: "nodes / edges の配列が必要です" });
  }
  if (nodes.length > 200 || edges.length > 400) {
    return res.status(400).json({ message: "ノード数が多すぎます" });
  }
  ...
  const safeNodes = nodes.map((n) => ({
    ...
    x: Number(n.x) || 0,
    y: Number(n.y) || 0,
    parentId: n.parentId == null ? null : String(n.parentId).slice(0, 40),
  }));
  ...
  await pool.query(
    `INSERT INTO note_mindmaps (note_id, user_id, data, source)
     VALUES (?, ?, ?, 'manual')
     ON DUPLICATE KEY UPDATE data = VALUES(data), source = 'manual', updated_at = CURRENT_TIMESTAMP`,
    [noteId, req.session.userId, JSON.stringify({ nodes: safeNodes, edges: safeEdges })]
  );
